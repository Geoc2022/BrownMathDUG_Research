...
def latex_to_markdown(latex_text):
    ...
    latex_text = re.sub(r'\\begin{enumerate}(.+?)\\end{enumerate}', list_items_to_markdown, latex_text, flags=re.DOTALL)
...
print(
    ...
    Note: After the conversion, there might have to be some changes made to the markdown file to gain full functionally. For example, embedded elements (like videos, images) must be added the markdown file after processing.
    """
)
...
try:
    ...
    try:
        ...
        try:
            ...
            post_foldername = f'{datetime.now().strftime("%Y-%m-%d")}---{title_input.replace(" ", "-")}'
